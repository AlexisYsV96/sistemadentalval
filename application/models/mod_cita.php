...
class mod_cita extends CI_Model {
    ...
    function get_cant_citas($estados,$anio,$mes=""){
        $this->db->select('*');
        $this->db->from('cita_medica');
        $this->db->join('horario_medico', 'cita_medica.codi_homed = horario_medico.codi_homed');
        $this->db->where_in('esta_cit', $estados);
        $this->db->where('YEAR(fech_homed)',$anio);
        if($mes != ""){
            $this->db->where('MONTH(fech_homed)',$mes);
        }
        return $this->db->count_all_results();
        //return $this->db->where_in('esta_cit', $estados)->from("cita_medica")->count_all_results();
    }

    public function get_cant_cita_por_mes($estados,$anio) {
        $this->db->select('MONTH(horario_medico.fech_homed) as mes, COUNT(*) as cantidad');
        $this->db->from('cita_medica');
        $this->db->join('horario_medico', 'cita_medica.codi_homed = horario_medico.codi_homed');
        $this->db->where_in('esta_cit', $estados);
        $this->db->where('YEAR(fech_homed)',$anio);
