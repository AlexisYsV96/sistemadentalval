...
$("#agregar_enf").click(function() {
    ...
    if (sw_enf) {
        ...
        num_diente_enf_t.push(diente+enfermedad);
...
$("#tabla_enfermedad").on("click", ".quitar_enfermedad", function() {
    var diente = $(this).parent().parent().find("td").eq(0).html();
var enfermedad = $(this).parent().parent().find("td").eq(1).html();
  var index = num_diente_enf_t.indexOf(diente+enfermedad);
  // var index = num_diente_enf.indexOf(diente);


   /*  var enfermedad = $(this).parent().parent().find("td").eq(1).html();
     var index = num_diente_enf.indexOf(diente+enfermedad);*/
    ...
    if (index > -1) {
        ...
        num_diente_enf_t.splice(index, 1);
        $(this).parent().parent().remove();
    }
...
$("#aceptar_piezas").click(function() {
    ...
    if ($("#pie_all").is(':checked') || $("#pie_mes").is(':checked') || $("#pie_dis").is(':checked') || $("#pie_ocl").is(':checked') || $("#pie_pal").is(':checked') || $("#pie_lin").is(':checked') || $("#pie_ves").is(':checked') || $("#pie_inc").is(':checked')) {
        ...
        var partes = "";
        var partes_desc = "";
        var xDientes = ["21", "22", "23", "24", "25", "26", "27", "28","31","32","33","34","35","36","37","38"];

        if ($("#pie_mes").is(":checked")) {
            partes += "M";
            partes_desc += " Mesial ";

            if (xDientes.includes(diente))
                select.find(".parte_izquierda").css("background", "blue");    
            else
                select.find(".parte_derecha").css("background", "blue");
        } else {
            partes += "0";
        }
